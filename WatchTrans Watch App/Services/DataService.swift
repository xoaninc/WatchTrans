...
//  UPDATED: 2026-03-28 - Replaced custom cache with Storage class
//
//  CACHING STRATEGY:
//  -----------------
//  • Stops/Lines/Colors: 24h TTL via Storage (JSON files with modification-date TTL)
//  • Location/Shapes: Infinite TTL (never expire)
//  • Arrivals: 45s in-memory only (no persistent cache)
//  • Platforms: In-memory only (per session)
//
        ...
        do {
            ...
            for i in stops.indices {
                ...
                stops[i] = Stop(
                    ...
                    accesibilidad: s.accesibilidad, bicycleParking: s.bicycleParking,
                    carParking: s.carParking, stopDescription: s.stopDescription,
